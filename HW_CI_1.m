...
for i = 1:10
    ...
    while(crr_epoch <= max_epoch)
        ...
        for j = 1 : length(data_rand)
            ...
            total_err(crr_epoch,i)= (dj - total_y(j,size(total_y,2)));
            
            %-----------------------back propagation
            %gradian output
            gd_out = DiffSigmoid(total_y(j,size(total_y,2)))*total_err(crr_epoch,i);
...
function [w1,w2,w3,w4] = GenerateW(x,y,z,a)
    w_rand_range = -(1/(sqrt(numel(x)+1))):0.00001: (1/(sqrt(numel(x)+1)));
